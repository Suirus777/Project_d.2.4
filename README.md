...
Потомучто решение KinD - это Kubirnet in Docker, KinD — это инструмент для запуска локальных кластеров Kubernetes с использованием «узлов» контейнера Docker. Так же Kubernetes в Docker использует подход Docker-in-Docker (DinD) для запуска кластера Kubernetes. И в других решениях кроме Docker KinD не сможет работать. KinD разварачивает кластера в контенерах Docker, Каждая нода кластера, будь то Control Plane-нода или worker ноды - это Docker контенеры в Docker контенере кластера. 
</b>
